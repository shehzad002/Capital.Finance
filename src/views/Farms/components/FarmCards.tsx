...
const FarmCards: React.FC = () => {
  ...
  const capIndex = farms.findIndex(
    ({ tokenSymbol }) => tokenSymbol === 'CAP',
  )

  const capPrice =
    capIndex >= 0 && stakedValue[capIndex]
      ? stakedValue[capIndex].tokenPriceInWeth
      : new BigNumber(0)

  const BLOCKS_PER_YEAR = new BigNumber(2372500)
  const CAP_PER_BLOCK = new BigNumber(50)
...
const FarmCard: React.FC<FarmCardProps> = ({ farm }) => {
  ...
  }, [cap, lpTokenAddress, account, setHarvestable])

  const poolActive = true // startTime * 1000 - Date.now() <= 0

  return (
    <StyledCardWrapper>
      {farm.tokenSymbol === 'CAP' && <StyledCardAccent />}
      <Card>
        <CardContent>
          <StyledContent>
